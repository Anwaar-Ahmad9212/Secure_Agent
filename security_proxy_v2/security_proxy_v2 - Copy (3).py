"""
security_proxy_v2.py - Advanced ML-Based Security Proxy (FIXED)
...
class SecurityProxyV2:
    ...
    def _load_config(self, path: str) -> dict:
        ...
        default_config = {
            "vector_db": {
                "path": "../security/vector_db",  # ← FIXED PATH
                "collection_name": "unified_malicious_prompts",
                "model_name": "all-MiniLM-L6-v2",
                "similarity_threshold": 0.70  # ← LOWERED from 0.75
            },
            "fuzzy_matching": {
                "enabled": True,
                "threshold": 85.0,
    ...
    def _initialize_detectors(self):
        ...
        self.vector_detector = VectorDetector(
            ...
            similarity_threshold=vector_config.get('similarity_threshold', 0.70)
        )
    ...
    def validate(self, prompt: str, source: str = "unknown") -> dict:
        ...
        # Layer 4: Vector similarity (with debug output)
        vector_result = self.vector_detector.detect(prompt)
        layer_results['vector_similarity'] = vector_result
        
        # Debug: Print vector similarity score
        if vector_result.get('available', False):
            sim = vector_result.get('max_similarity', 0)
            threshold = vector_result.get('threshold', 0.70)
            print(f"[DEBUG] Vector similarity: {sim:.3f} (threshold: {threshold})")
            if sim >= threshold:
