...
"""
vector_detector.py - Layer 4: Vector DB Similarity (UPDATED)

Fixed zero similarity issue, proper embedding handling and distance-to-similarity conversion.
Uses ChromaDB vector database for semantic similarity detection.
"""
...
class VectorDetector:
    ...
    Uses existing 'unified_malicious_prompts' collection.
    Handles Euclidean or cosine distances correctly.
    """
    ...
    def detect(self, prompt: str) -> Dict:
        """
        Detect semantic similarity to known malicious prompts.
        Returns a dictionary with score, similarity, matched prompt, category, etc.
        """
        if not self.available:
            return {
                'score': 0.0,
        ...
        try:
            # Proper embedding type: numpy array
            ...
            if results['distances'] and results['distances'][0]:
                ...
                similarity = 1 / (1 + distances[best_index])
                score = similarity

                # Debug top 3 matches
                print(f"[Vector] Top {min(3,len(distances))} matches:")
                for i in range(min(3, len(distances))):
                    sim_i = 1 / (1 + distances[i])
                    doc_i = results['documents'][0][i][:60]
                    cat_i = results['metadatas'][0][i].get('category', 'unknown')
                    print(f"   {i+1}. Sim: {sim_i:.3f} | Cat: {cat_i} | '{doc_i}...'")
